...
def _randomize_samples(targets, _class, n_samples=1):
    ...
    class '_class'
    """
    assert len(list(filter(lambda x: x == _class, targets))) >= n_samples, \
        f"There's not enough samples of class {_class} in targets " +\
...
def generate_all_plots(model, features, feature_names, targets, output_dir='explanations', n_samples=3):
    ...
    assert len(shap_values) == len(classes), \
        "Classes generated by the explainer of 'model' doesn't match the distinct number " +\
