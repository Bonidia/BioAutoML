...
def _randomize_samples(targets, _class, n_samples=1):
	"""

	Randomize Function: Get a given number of samples which match with
	class '_class'

	"""
	if len(list(filter(lambda x: x == _class, targets))) < n_samples:
		raise ValueError(
	...
	while len(samples) < n_samples:
		...
		if targets[rng] == _class:
			samples.append(rng)
		
	return samples


def _generate_waterfall_plot(id_plot, class_shap_values, class_name, sample_row, 
							base_value, feature_names, path):
	"""
	
	Generate Waterfall Plot (Private): Generates a waterfall plot for a given sample
...
def _generate_summary_plot(class_shap_values, class_name, data, feature_names, path):
	...
	Generate Summary Plot (Private): Generates a summary plot for a given class of
...
def generate_all_plots(model, features, feature_names, targets, output_dir='explanations', n_samples=3):
	...
	Generate All Plots Function: Used to generate each of the plots used to
	...
	# need to assert if this still makes sense
	classes = list(OrderedSet(targets))
	...
	print("Plotting each class with summary plot...")
	for i in range(len(classes)):
		_generate_summary_plot(shap_values[i], classes[i], features, feature_names, path)

	print("Plotting each class with waterfall plot...")
	for i, cl in enumerate(classes):
		random_samples = _randomize_samples(targets, cl, n_samples=n_samples)
...
def multiclass_pipeline(test, test_labels, test_nameseq, norm, classifier, tuning, output):
	...
	if os.path.exists(ftest) is True:
		...
		try:
			...
			print(e)
			print("Explanations plots failed. Ignoring them...")
		else:
			print("Explanations plots generated successfully!")
